...
package org.igniterealtime.xiff.data.whiteboard
{
	...
	public class Path implements ISerializable
	{
		...
		{
			var node:XML = <{ Path.ELEMENT_NAME }/>;
			node.@p = serializeSegments();
		...
		private function loadCommand( cmd:String, str:String ):void
		{
			...
			while ( params.length > 0 )
			{
				...
				else
				{
					...
					{
						case "M":
							_lastLocation = { x: Number( params.shift() ), y: Number( params.shift() ) };
							break;

						case "L":
							current = { x: Number( params.shift() ), y: Number( params.shift() ) };
							addSegment( { from: _lastLocation, to: current } );
							_lastLocation = current;
							break;

						case "l":
							current = { x: _lastLocation.x + Number( params.shift() ), y: _lastLocation.y +
										Number( params.shift() ) };
							addSegment( { from: _lastLocation, to: current } );
							_lastLocation = current;
							break;
