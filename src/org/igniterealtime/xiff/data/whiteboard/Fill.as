...
package org.igniterealtime.xiff.data.whiteboard
{
	...
	public class Fill implements ISerializable
	{
		...
		public function serialize( parent:XML ):Boolean
		{
			if ( _color )
			{
				parent.@fill = "#" + _color.toString( 16 );
			}
			if ( _opacity )
			{
				parent.@["fill-opacity"] = _opacity.toString();
			}
		...
		public function deserialize( node:XML ):Boolean
		{
			if ( node.@fill )
			{
				_color = parseInt( "0x" + node.@fill.slice( 1 ) );
			}
			if ( node.@["fill-opacity"] )
			{
				_opacity = parseFloat( node.@["fill-opacity"] );
			}
		 ...
		 * @see http://livedocs.adobe.com/flex/3/langref/flash/display/Graphics.html#beginFill()
		 */
		 ...
		 * This property / 100 can be used as the alpha parameter of Graphics.beginFill().
		 *
		 */
