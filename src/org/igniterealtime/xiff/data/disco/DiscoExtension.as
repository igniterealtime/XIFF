...
package org.igniterealtime.xiff.data.disco
{
	...
	public class DiscoExtension extends Extension implements ISerializable
	{
		 ...
		 */
		public function DiscoExtension( xmlNode:XML )
		{
			super( xmlNode );
		}
		...
		/**
		 * @see http://www.igniterealtime.org/issues/browse/XIFF-51
		 */
		...
		public function get service():EscapedJID
		{
			...
			if ( parent.@type == IQ.TYPE_RESULT )
			{
				return new EscapedJID( parent.@from );
			}
			else
			{
				return new EscapedJID( parent.@to );
			}
		}
		public function set service( value:EscapedJID ):void
		{
			...
			if ( parent.@type == IQ.TYPE_RESULT )
			{
				parent.@from = value.toString();
			}
			else
			{
				parent.@to = value.toString();
			}
		}
	}
}
