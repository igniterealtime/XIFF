...
package org.igniterealtime.xiff.data.forms
{
	...
	public class FormExtension extends Extension implements IExtension, ISerializable
	{
		...
		private var myInstructionsNode:XML;

		private var myReportedFields:Array = [];

		private var myTitleNode:XML;
		...
		public function deserialize( node:XML ):Boolean
		{
			...
			for each ( var c:XML in node.children() )
			{
				...
				switch ( c.name() )
				{
					...
					case "reported":
						for each ( var reportedFieldXML:XML in c.children() )
						{
							field = new FormField();
					...
					case "item":
						...
						for each ( var itemFieldXML:XML in c.children() )
						{
							field = new FormField();
		...
		{
			if ( myTitleNode && myTitleNode[0] )
			{
				return myTitleNode[0].toString();
			}
